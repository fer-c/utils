...
%% update_validator_4_test() ->
%%     ?assertError(
%%         #{code := invalid_value},
%%         maps_utils:validate_update(
%%             #{x => 2}, #{x => 5},
%%             #{x => #{update_validator => fun(X, Y) -> Y == 2*X end}}
%%         )
%%     ).
...
%%                 required => false, 
%%                 default => <<"foo">>,
%%                 update_validator => fun(_, _) -> true end
%%                 }
%%             }
%%         )
%%     ).
...
%%                 required => false, 
%%                 default => <<"foo">>,
%%                 update_validator => fun(_, _) -> false end
%%                 }
%%             }
%%         )
%%     ).
...
%% update_merge_test() ->
%%     Result = maps_utils:validate_update(
%%         #{foo => 1, bar => 2}, 
%%         #{
%%             foo => #{
%%                 key => foobar,
